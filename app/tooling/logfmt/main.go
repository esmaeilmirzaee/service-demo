// This program takes the structured log output and make it readable
package main
...
func main() {
	...
	for scanner.Scan() {
		...
		// I want them in.
		b.Reset()
		b.WriteString(fmt.Sprintf("%s: %s: %s: %s: %s: %s", m["service"],
			m["ts"], m["level"], traceID, m["caller"], m["msg"]))

		// Add the rest of the keys ignoring the ones we already
		// added for the log.
		for k, v := range m {
			switch k {
			case "service", "ts", "level", "traceid", "caller", "msg": continue
			}
